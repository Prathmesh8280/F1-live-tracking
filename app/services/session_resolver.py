...
async def resolve_race_session():
    ...
    # Walk backwards from the current year until we find a session with data.
    # Typically resolves on the first iteration (current year); falls back to
    # previous year only when the season hasn't started yet (e.g. January).
    for year_offset in range(3):
        year = now.year - year_offset
        sessions = await openf1_client.get_sessions(year=year)
        if not sessions:
            logger.info("No sessions found for %d — trying previous year.", year)
            continue
        ...
        # If we found sessions for this year but none had data, try the previous year.
        logger.info("No sessions with data found for %d — trying previous year.", year)
